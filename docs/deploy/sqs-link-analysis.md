...
- 구조를 이해하려면 [1~4절](#1-어떤-문제를-해결하려고-바꿨는가): 해결할 문제 → 용어 → 링크 하나의 처리 → 장애 복구.
- PC에서 실행하는 방식을 보려면 [5절](#5-pc-워커는-어떻게-연결하는가): 수동 전환과 수집기 교체.
- 코드를 확인하려면 [6절](#6-구현-참고): 파일별 책임과 테이블.
- 실제 실행·배포가 필요하면 [7~9절](#7-실행과-수동-워커-전환): 설정, 운영 절차, 검증 명령.
...
## 2. Worker·SQS·Job·Outbox는 각각 무엇인가
...
### Outbox: 링크 저장과 요청 전달 사이의 틈을 막는 기록
...
### 워커 종료: SQS 대기 시간과 Job 만료 시간으로 복구한다
