...
링크 스냅샷에 대한 AI 요약 메트릭 테이블이다. 서비스에 반영되는 요약은 `link_snapshots.ai_summary`에 저장하고, 각 요약 시도에서 생성된 요약문, 모델, 토큰, 비용, 소요시간, 실패 사유는 이 테이블에 누적한다.
...
```mermaid
erDiagram
  LINK_SNAPSHOTS ||--o{ AI_SUMMARY_METRICS : has
...
| link_snapshot_id | bigint | Y | 요약 대상 링크 스냅샷 ID. 물리 FK 없이 논리 참조로 저장 |
| attempt_number | integer | Y | 같은 스냅샷 내 요약 시도 순번. 최초 시도는 `1` |
...
| prompt_key | varchar | N | 요약 프롬프트 식별 키. 버전이 필요한 프롬프트는 키에 버전을 포함 |
...
- 같은 스냅샷 안에서는 `link_snapshot_id + attempt_number`가 유니크해야 한다.
- `status = SUCCESS`인 메트릭의 `generated_summary`를 `link_snapshots.ai_summary`의 기본 채택 대상으로 본다.
...
- `ai_summary_metrics.status`는 개별 요약 시도의 상태이며, 스냅샷 단위 대표 상태는 `link_snapshots.ai_summary_status`에 저장한다.
...
- 서비스에 반영된 요약은 `link_snapshots.ai_summary_metric_id`가 가리키는 메트릭 row의 `generated_summary`와 같아야 한다.

## 인덱스 설계

```sql
CREATE UNIQUE INDEX ai_summary_metrics_snapshot_attempt_idx
  ON ai_summary_metrics (link_snapshot_id, attempt_number);

CREATE INDEX ai_summary_metrics_snapshot_status_idx
  ON ai_summary_metrics (link_snapshot_id, status);

CREATE INDEX ai_summary_metrics_model_created_at_idx
  ON ai_summary_metrics (model_provider, model_name, created_at);
```

- `link_snapshot_id + attempt_number`: 같은 스냅샷의 재시도 순번 중복 방지.
- `link_snapshot_id + status`: 특정 스냅샷의 성공/실패 요약 시도 조회용.
- `model_provider + model_name + created_at`: 모델별 비용/성능 집계용.

## 재시도 정책

- 최초 실행 실패 시 1회 재시도한다.
- 재시도까지 실패하면 해당 스냅샷의 `ai_summary_status`를 `FAILED`로 전환한다.
