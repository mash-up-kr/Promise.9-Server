...
`purpose`는 이 토큰이 어떤 발급 경로로 만들어졌는지 구분한다. `primary`는 소셜 로그인(`POST /auth/social`)으로 직접 발급된 토큰, `extension`은 `POST /auth/extension-token`이 primary 세션을 대신해 위임 발급한 토큰이다. 회전(`POST /auth/refresh`) 시에도 이 값을 그대로 이어받는데, 그렇지 않으면 탈취된 extension 토큰이 refresh 한 번으로 primary 권한을 얻어버린다.
...
| purpose | varchar | Y | 발급 경로. `primary`(소셜 로그인 직접 발급) \| `extension`(extension-token으로 위임 발급). 기본값 `primary` |
...
- 회전 시 이전 토큰의 `revoked_at`을 채우고 같은 `token_family`, `purpose`로 새 토큰을 발급한다.
- 폐기된 토큰이 재사용되면 해당 `token_family`의 모든 토큰을 revoke한다.
- `purpose=extension`인 토큰으로는 `POST /auth/extension-token`을 다시 호출할 수 없다 (재귀 발급 차단).
