...
| attempt_number | integer | Y | 같은 링크와 task type 내 metric row 순번 |
...
```sql
CREATE UNIQUE INDEX ai_metrics_user_link_task_attempt_idx
  ON ai_metrics (user_link_id, task_type, attempt_number);

CREATE INDEX ai_metrics_user_link_task_status_idx
  ON ai_metrics (user_link_id, task_type, status);
```

- `user_link_id + task_type + attempt_number`: metric row 순번 중복 방지
...
호출 유스케이스가 다시 `generateText` 또는 `generateObject`를 호출하면 새 `attempt_number`로 기록된다.

`attempt_number`는 metric row 순번이므로 단독으로 비즈니스 retry 실행 단위를 나타내지 않는다.
비즈니스 retry 제한이 필요하면 호출 유스케이스가 별도 실행 식별자와 정책을 관리한다.

metrics 기록은 best-effort다.
동시 insert unique 충돌은 제한된 횟수만 재시도하며, 최종 저장 실패는 로그만 남기고 LLM 결과나 원래 오류를 덮지 않는다.
