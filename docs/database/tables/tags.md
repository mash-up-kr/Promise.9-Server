...
CREATE INDEX tags_normalized_name_trgm_idx
  ON tags USING gin (
    regexp_replace(lower(coalesce(normalized_name::text, '')), '[[:space:]]', '', 'g') gin_trgm_ops
  );
```

- `link_id + normalized_name`: 한 저장 링크 안에서 같은 태그 중복 추가 방지.
- `user_id + normalized_name`: 사용자 내부에서 같은 태그를 가진 링크를 찾는 exact match 추천/검색용.
- `normalized_name gin_trgm_ops`: 검색어의 태그 부분일치 후보 회수용. 마이그레이션 `0006_link_text_search.sql`에서 `pg_trgm` 확장과 함께 적용한다.
