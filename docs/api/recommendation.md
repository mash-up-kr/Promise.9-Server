...
사용자의 활성 링크를 기준으로 링크가 많이 저장된 폴더와 태그를 하나의 flat 목록으로 반환한다. 개별 폴더·태그에 활성 링크가 3개 이상 연결되어야 추천 후보가 되며, 홈 화면에서는 query를 생략해 기본값인 최대 12개를 사용한다.

| Query   | 타입   | 기본값 | 설명                                |
| ------- | ------ | ------ | ----------------------------------- |
| `limit` | number | `12`   | 반환할 최대 개수. 최소 1, 최대 50개 |
...
서버는 활성 링크가 3개 이상 연결된 폴더·태그만 후보로 추린 뒤, `limit`을 적용하기 전에 전체 후보 수를 확인한다.

- 후보가 3개 이상이면 정렬 후 최대 `limit`개를 `data.items`로 반환한다.
- 후보가 2개 이하면 다음과 같이 `data: null`을 반환한다.
...
- 태그는 `normalizedTag`가 같은 항목을 하나로 집계하며, 표기명이 여러 개면 사전순으로 가장 앞선 값을 `label`로 사용한다.
- soft delete된 링크와 삭제된 폴더는 집계에서 제외한다.
- 활성 링크가 1~2개인 폴더와 태그는 추천 후보에서 제외한다.
