...
홈에서는 `GET /recommendations`를 query 없이 호출해 기본 최대 12개를 요청합니다. 폴더와 태그는 `items`의 한 목록에 섞여 있으며 `label`을 키워드 칩에 표시합니다.

- 전체 폴더·태그 후보가 4개 이상: `data.items`로 섹션을 구성합니다.
- 전체 후보가 3개 이하: API가 `data: null`을 반환하며 섹션 제목과 칩을 모두 렌더링하지 않습니다.
