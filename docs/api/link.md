...
- 같은 사용자가 이미 저장한(삭제되지 않은) 동일 URL(정규화 기준)을 다시 저장하면 `409`(`930003`)로 거부한다.
    - 이때 `error.linkId`에 기존 링크 ID가 포함되므로, 새로 만들지 않고 그 링크로 이동시키는 등의 처리에 쓸 수 있다.
...
```json
{
    ...
    "error": {
        ...
        "timestamp": "2026-07-13T09:41:00.000Z",
        "linkId": 42
    }
}
```
