...
| 필드           | 규칙                                                                                       |
| -------------- | ------------------------------------------------------------------------------------------ |
| `summary`      | 한국어 요약. 프롬프트는 300자 내외를 요구하고 스키마 상한은 500자다. NFC로 정규화하고 앞뒤 공백을 제거해 반환한다. 결합되지 않은 한글 자모나 한자·일본어·키릴 같은 다른 문자 체계가 남으면 `AI_GENERATED_TEXT_NOT_KOREAN` 재시도 가능 오류로 던진다. |
| `tags`         | 최대 5개, 각 20자 이내. 넓은 범주에서 세부 개념 순서로 정렬된다. 요약과 같은 한국어 문자 검사를 거친다. |
