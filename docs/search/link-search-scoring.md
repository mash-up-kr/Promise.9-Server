...
검색어를 소문자화하고 Unicode 문자·숫자 토큰으로 나눈다. NFKC 호환 정규화는 적용하지 않으며, 키워드 후보 조회와 키워드 점수에는 앞의 최대 12개 토큰을 동일하게 사용한다. 다음 네 경로에서 각각 최대 30개를 회수한다.
...
키워드 경로는 대상 필드를 소문자화하고 공백만 제거한 표현식에 `LIKE '%token%'`을 적용한다. 점수 계산도 같은 대상 표현식을 사용하므로 구두점 경계를 후보 단계와 다르게 제거하지 않는다. `pg_trgm` GIN 표현식 인덱스는 제목, 본문, 태그에 각각 존재한다. 1~2자 검색어는 추출할 trigram이 없어 사용자 범위 스캔으로 퇴화할 수 있다.

후보 합집은 최대 120개이며, 이 후보 풀을 재랭킹한 상위 30개만 노출한다. 전체 사용자 범위의 exact top 30을 보장하는 구조는 아니다. API `limit`은 기본 9, 최대 30이며 이 30개 안에서 cursor 페이지네이션한다.
...
| `tagKeyword`     | 검색 토큰 중 개별 태그 하나에 부분일치하는 비율 |        0.20 |
