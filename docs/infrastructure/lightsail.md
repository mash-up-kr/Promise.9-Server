...
CDK는 서버 내부의 Docker, Nginx, PostgreSQL 데이터, 파일과 환경변수를 관리하거나
컨테이너를 실행하지 않는다. 애플리케이션과 DB 배포는
[Lightsail Docker Deployment](../deploy/lightsail-docker.md)를 따른다.
...
- API와 PostgreSQL이 같은 Instance를 사용하므로 Instance 교체는 API와 DB를 모두
  중단시킨다. DB 백업을 Instance 외부에도 보관해야 한다.
