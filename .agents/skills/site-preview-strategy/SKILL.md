...
- **본문:** 작성자·UI·반응 수를 제거하되 본문 속 일정과 게시 시각을 혼동하지 않는다. 반복 블록과 주변 구조를 확인한다. 메타 설명의 완전성을 확인하지 못하면 collect의 전문 대체값으로 사용하지 않는다. 현재 X는 본문 경계나 설명과의 대응을 확인할 수 없으면 재시도 가능한 `TinyFishFetchError`를 반환한다. preview의 요약 사용과 collect의 전문 요구를 구분하고, 실패·대체 정책은 해당 사이트의 현재 코드·테스트를 따른다. 정책을 변경해야 한다면 이유와 저장·AI 입력에 미치는 영향을 먼저 확인한다.
...
- **순번:** `image_links` 배열 위치를 사진 번호로 해석하지 않는다. `/photo/N` 같은 실제 미디어 링크로 대응한다. 명시된 순번에 대응하는 이미지를 확보하지 못하면 다른 사진으로 대체하지 않고 `null`을 반환한다. 순번을 지정하지 않은 URL에서만 같은 콘텐츠의 대표 이미지를 선택한다. 구체적인 매핑·실패 정책은 해당 사이트의 현재 코드·테스트를 확인한다.
