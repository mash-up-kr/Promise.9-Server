...
// 후보 조회와 같이 공백을 제거해 `머신러닝`↔`머신 러닝`, `nest`↔`nestjs`가
// 후보에만 들어오고 lexical 점수는 0이 되는 불일치를 막는다.
export function queryTokenCoverage(
    ...
    const normalizedTarget = tokenizeLinkText(target).join('')
    let matches = 0
...
export function calculateSearchSignals(
    ...
    return {
        ...
        tagKeyword: queryTokenCoverage(query, candidate.tags?.join(' ')),
