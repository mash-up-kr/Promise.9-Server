...
### Instagram Reel 표지 보완

`/reel/{shortcode}/`와 `/reels/{shortcode}/`는 원본 URL에서 제목·본문을 먼저 수집한다.
원본 결과에 대상 표지가 없으면 같은 TinyFish Fetch API로
`https://www.instagram.com/reel/{shortcode}/embed/`를 한 번 추가 조회한다.
embed의 제목·본문은 원본 결과를 덮어쓰지 않는다.

embed에도 추천 이미지가 섞이므로 Instagram·Meta HTTPS CDN의 콘텐츠 이미지 중
`ig_cache_key`를 디코딩한 값이 대상 shortcode의 미디어 ID와 일치하는 후보만 사용한다.
실제 응답에서 관측된 `미디어 ID + 17자리 보조 식별자` 형식도 전체 길이와 숫자 형식을 검사한 뒤
미디어 ID를 정확히 비교한다. 보조 식별자는 이미지 파일명의 ID와 다를 수 있다.
ID 접두사나 `ig_cache_key` 존재 여부만으로는 선택하지 않는다. 이 형식은 관측에 기반한
규칙이므로 Instagram 응답 형식이 바뀌면 재검증이 필요하다.

원본이 수집 불가·오류이면 보완 요청을 하지 않는다. 표지가 이미 있거나 일반 게시물·프로필·X
링크이면 추가 조회하지 않는다. 보완 요청만 실패하거나 일치하는 표지가 없으면 이미지는
null로 두고 원본 수집 결과를 유지한다. 이미지 보완 실패는 별도 재시도 작업으로 발행하지 않는다.
추가 요청에는 기존 TinyFish 요청 제한(25초)이 적용되므로 표지가 없는 릴스는 응답 시간이
늘어날 수 있다. 최종 이미지에는 기존 공개 URL 검사와 URL 길이 제한을 그대로 적용한다.
