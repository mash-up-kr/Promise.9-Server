...
// 링크 분석 Job 큐. 애플리케이션의 SQS_LINK_ANALYSIS_QUEUE_URL이 이 큐를 가리킨다.
const QUEUE_NAME = 'promise9-link-analysis'
// 시도 횟수는 코드의 JOB_MAX_ATTEMPTS가 제어하므로, redrive는 파싱 실패와
// DB 장애처럼 처리 자체가 불가능한 메시지만 걸러내는 안전망 역할이다.
...
// 메시지에는 Job ID만 담으며 저장 암호화와 HTTPS 전송을 기본값에 맡기지 않는다.
...
// 링크 분석 Job 큐와 DLQ, 그리고 런타임이 사용할 IAM 사용자를 정의한다.
